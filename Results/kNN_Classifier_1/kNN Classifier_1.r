...
path <- "C:\\Users\\Eric\\Dropbox\\SVN_FILES\\School Projects\\Chuan Self Study\\Sample Data\\data-gold1-Guardino.json"
c <- file(path, "r")
l <- readLines(c, -1L)
...
path <- "C:\\Users\\Eric\\Dropbox\\SVN_FILES\\School Projects\\Chuan Self Study\\Sample Data\\data-gold2-Guardino.json"
c <- file(path, "r")
l <- readLines(c, -1L)
json <- lapply(X=l, fromJSON)
myList <- list()
...
TrainingData <- as.data.frame(lapply(TrainingData[, 1:length(TestData)-1], normalize))
TestData <- as.data.frame(lapply(TestData[, 1:length(TestData)-1], normalize))
...
{
  Results[i] <- crossTable[i,i]/sum(crossTable[,i])
}
